...
> **段階 1（定義 + インスタンス）まで実装済み**。
> 段階 2（型付きパラメータと式）が本体なので、そこまでやらないと完成しない。
