...
6. **パラメータは GUI のアクションではなくテキストの式**（ADR-0031）。
   ノードエディタは「GUI でプログラミングさせる」点でアクションと同じ罠なので採らない。
   **式の中の角度は度**。`NaN` や無限大は作らせずエラーで止める
   （座標へ流れると図形が消えて原因が追えない）
7. **日常操作は打たせない**（ADR-0032）。値の変更・配置・リセットはパネルで
   マウス操作。文字を打つのは名前を付けるときだけ
8. **コンポーネントの定義は「実エンティティ + 疎な式の束縛」。**
   `Geometry` を式化した並行型階層にしない（定義の中で既存の作図ツールが
   使えなくなる。ADR-0029）。`Geometry::bbox` / `dist_to` / `is_bounded` は
   `&DefinitionTable` を引数で受け取る。**`Aabb::EMPTY` で逃げない**（ADR-0030）
9. **ファイル形式は 2 つで役割が分かれている。** `.ymc`（ネイティブ・無損失）が
   保存形式、`.dxf`（R12・非可逆）は交換専用。**DXF を保存形式に戻さない。**
   形式は**拡張子だけ**で決める（`crates/cad-app/src/file_ops.rs` の `is_dxf`）
10. **`cad-core` の依存パッケージはゼロ。** 増やす前に必ずユーザーへ確認する
...
（3D / OFFSET / ハッチング / 寸法記入 / 文字要素 / 外部参照 / 幾何拘束ソルバ /
印刷 / DWG / Windows・macOS / Web・wasm）。

> TRIM / EXTEND / FILLET / CHAMFER は Issue #7 で**線分のみ**実装済み。
> ポリラインの部分トリムと、円・円弧を含む角は残課題。
>
> ブロックは Issue #13 で「コンポーネント」として再定義し、
> **段階 1（定義 + インスタンス）と段階 2（型付きパラメータと式）が実装済み**。
> 残るは段階 3（インプレース編集）と、操作をパネルへ寄せる改善（Issue #15）。
