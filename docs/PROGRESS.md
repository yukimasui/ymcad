...
| 段階 | 内容 | 状況 |
|---|---|---|
| 1 | 定義とインスタンス | ✅ ユーザー確認済み・`main` へマージ済み（PR #14） |
| 2 | 型付きパラメータと式 | ✅ ユーザー確認済み |
| 3 | インプレース編集 | ✅ 実装済み（ユーザー確認済み） |

> **パラメータパネルは段階 3 から前倒しした。**
> 動作確認で「日本語入力に難がありすぎて使いづらい」との指摘を受けたため
> （ADR-0032）。機能が動くことと使えることは別で、後者が欠けていれば実質未完成。
...
- **`tan(90)` は `is_finite` では捕まらない。** 90 度をラジアンにしても厳密に
  π/2 にならないので、無限大ではなく `1.6e16` という巨大な有限値になる。
  それが座標へ流れると図形が事実上消える。`cos` が 0 かで判定する
- **入力の正規化が式と名前を壊す。** `Session::interpret` は Word を
  大文字化して `normalize_ascii` をかける。オプション（`C` / `2P`）には
  正しいが、`if` が `IF` になって式が通らず、`データー` の長音が `-` に
  直されて名前が壊れる。`Tool::wants_raw_text()` で状態ごとに切り替える
- **パラメータの型変更が抜け道になっていた。** 「束縛が参照しているパラメータは
  消せない」は入れたのに、**名前を残して型だけ変える**と素通りして、
  束縛の式が数値でなくなり座標が黙って定義のままになっていた
- **`cargo fmt` が行を畳むと、文字列一致の置換が当たらなくなる。**
  これで「v3 のパラメータを書いているのに読んでいない」状態を作った。
  Rust 側に「末尾でぴったり尽きる」検査が無かったので、
  ラウンドトリップテストが見逃した（`validate_ymc.py` は最初から見ていた）。
  **検査を Rust 側にも入れた**（入れたら読み飛ばしで 6 本落ちる）
- **テストの値が偶然一致していた。** 上書きを `幅 = 1800` にしていたが、
  `半幅 = 900` が既定の `幅 = 900` と一致して「効いていない」ことを見逃す形だった

### 段階 3 で分かったこと

- **束縛の添字問題（ADR-0029 の代償）は、定義の形を変えずに解けた。**
  編集セッションの間だけ「置いた要素 → 元の添字」の対応表を持てばよい。
  永続化する形を増やさずに済んだ
- **印を持たずに編集中の要素を追える。** スロット番号が再利用されないので、
  入った時点の最大番号より大きいものが「あとから作られた」と分かる。
  印を持たないと Undo で自動的に辻褄が合う（削除を取り消すと元の ID で戻るため）

### Issue #15（パラメータ操作のパネル化）— 完了

日本語入力が辛いという指摘への答え。**打つのは名前だけ**にした。

- [x] `BIND` をクリックで指す（座標を打たない）
- [x] パラメータの宣言をパネルで完結（名前を打つ → 型を選ぶ → 既定値をドラッグ → 宣言）
- [x] 宣言の削除ボタン
- [x] 効いている束縛の一覧（`LINE[0] の 終点X ← 幅`）
- [x] 同じ定義のインスタンスを複数選んでまとめて編集

**束縛を見せるために `Expr` の `Display` を書いた。**
括弧の付け方を間違えると意味が変わる（`10 - (3 - 2)` の括弧を落とすと 9 が 5 になる）ので、
**書き出して読み直すと同じ木になること**を往復テストで固定してある。

対象の定義は「編集中 → 選択中のインスタンス」の順で決める。
**パネル専用の選択状態を増やさない**（増やすと図面側の選択と食い違う）。

### 段階 3 / Issue #15 で踏んだ落とし穴

- **`mod tests` のあとに実装を足して `items after a test module` を出した。3 回目。**
  下の「既知の落とし穴」に書いてあるのに繰り返した。
  **テストは常にファイル末尾**。実装を足すときはスクロールして位置を確かめる
- `ids().next()` で編集対象を選ぼうとして、**別のインスタンス**を拾った。
  編集で 1 つ目のスロットが空いたため。`EditSession::members` から選ぶこと
- **エイリアス `CP` が `COPY` と衝突**した。`CS` にした。
  重複検査テスト（`command_names_and_aliases_are_unique`）が捕まえた

### 積み残し
- `PURGE`（未使用の定義を消す）コマンドは `DeleteDefinition` があるが UI が無い
- **編集中に描いた図形はその場では束縛できない**（定義に入っていないため）。
  `ENDCOMP` で確定してからになる
- **束縛の編集はまだパネルからできない**（一覧で見えるだけ。直すには `BI` で上書き）

### 修正済み: インスタンスのラバーバンドが描かれない（2026-08-13）

**症状。** コンポーネントのインスタンスを `MOVE` すると確定前のプレビューが出ない。
**単発の図形なら出る。**

**原因。** `render.rs::draw_geometry` に足した空実装。
...
**このコメントが誤り。** `draw_preview` も同じ `draw_geometry` を通る。
インスタンスを `MOVE` するとラバーバンドは `Geometry::Instance`（配置をずらしたもの）
として返るので、この腕に落ちて何も描かれなかった。
単発の図形が平気だったのは `Line` などの別の腕を通るから。

**`Aabb::EMPTY` で逃げるな（ADR-0030）と書いておきながら、描画側で同じことをやった。**
「呼び出し元は 1 つだけ」という思い込みが根本原因。

**修正。** `draw_geometry` / `draw_preview` に `&DefinitionTable` を渡し、
インスタンスは中身へ展開して再帰する。ADR-0030 と同じ原則。

**再発防止。** `egui::Painter` が実際に図形を積んだかを数えるテストを 6 本追加した。
`end_pass()` でクリップ済みの図形列を受け取って数える（`GraphicLayers` の中身は
非公開なので）。`TexturesDelta` は未適用のまま drop すると panic するので明示的に
`clear` する。**空実装に戻すと 2 本が落ちることを確認済み。**

**調査で遠回りした点（次に活かす）。**
最初の報告は「MOVE のプレビューが出ない」だったので全経路を疑い、
`Session::preview` の回帰テストを 7 本書いたが全部通ってしまい行き詰まった。
**「単発の図形は出る」という一言で即座に切り分けられた。**
症状の切り分け（**何なら動くか**）を先に聞くべきだった。

### ユーザーの目視確認が必要な項目

段階 1・2 とも 2026-08-13 にユーザー確認済み。

- [x] **インスタンスを `MOVE` したときにラバーバンドが出るか**（修正後、確認済み）
- [x] `PA` → `BI` → パネルで値を変えると形が変わるか
- [x] パネルの「配置」「戻す」が効くか
