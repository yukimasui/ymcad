...
### 積み残し
- **段階 3（インプレース編集）。** 定義の中身を画面に出して編集する
- **`BIND` の操作が重い**（要素番号とスロット名を打たせている）。
  定義の中身を画面に出せばクリックで指せる。段階 3 と仕組みが共通なので
  一緒にやる（[Issue #15](https://github.com/yukimasui/ymcad/issues/15)）
- **`PARAM` のパネル化**（Issue #15）
- **束縛の添字問題**（ADR-0029 の代償）。中身と束縛を必ず一緒に持ち替える形で
  「指す先の無い束縛は存在しない」ことは保証したが、**中身を差し替えると
  束縛は捨てられる**。インプレース編集では「編集しても束縛を保つ」ことが要るので、
  定義内で安定な ID をエンティティへ持たせる必要がある
