...
### ユーザーの目視確認が必要な項目
- [ ] 選択 → `B` でその場でインスタンスに置き換わるか（図形が消えないか）
- [ ] `I` で複数配置でき、回転（度）と倍率が効くか
- [ ] **`RD` で定義を差し替えると全インスタンスが変わるか**
- [ ] インスタンスに `MOVE` / `RO` / `SC` / `MI` が効くか（特に **`MI` の鏡像**）
- [ ] インスタンス内部の端点・中点にスナップできるか
- [ ] `X` で中身へ分解され、中身のレイヤに戻るか
- [ ] `.ymc` で保存 → 開き直して定義・入れ子・反転が保たれるか
- [ ] `.dxf` で保存 → `BLOCKS`/`INSERT` になり、パラメータ喪失の警告が出るか
- [ ] `U` ですべて戻るか（`COMPONENT` は 1 回で戻ること）
